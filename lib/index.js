...
function NwBuilder(options) {
    ...
    var defaults = {
        ...
        armDownloadUrl: 'http://dl.nwjs.io/',
        buildType: 'default',
        forceDownload: false,
        macCredits: false,
        macIcns: false,
...
NwBuilder.prototype.mergeAllFiles = function(files, platformName) {
    var self = this;
    var platformFiles = [];
    for (var m in this.options.platformFiles) {
        if (this.options.platformFiles[m].hasOwnProperty(platformName)) {
            platformFiles.push(this.options.platformFiles[m][platformName]);
        }
    }
    return new Promise(function(resolve, reject) {
        ...
        Utils.getFileList(platformFiles).then(function(data) {
            var mfiles = data.files;
...
NwBuilder.prototype.downloadNodeWebkit = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if(name === 'linuxARM') {
            platform.url = self.options.armDownloadUrl;
        }
...
NwBuilder.prototype.preparePlatformSpecificManifests = function(){
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if (overrides[name] || overrides[name.substr(0, name.length-2)]) {
            if (name === 'linuxDev') {
                // TODO(hamza) Yet another stupid hack to make dev tools work.
                // Ideally we need to modify platform-overrides as well as nw-builder
                // packages to support *-dev platforms.
                platform.platformSpecificManifest = self._appPkg;
                _.extend(platform.platformSpecificManifest, self._appPkg.platformOverrides[name]);
                return;
            }
            platformOverrides({
        ...
        new Promise(function(resolve, reject) {
            ...
            .then(function(platformAgnosticZip){
                ...
                _.forEach(self._zips, function(zip, platformName){
                    ...
                    zipPromises.push(new Promise(function(resolve, reject) {
                        self.mergeAllFiles(self._files, platformName).then(function(files) {
